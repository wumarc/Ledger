...
import com.Ledger.ledger.Adapter.TransactionAdapter;
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                if (snapshot.exists()) {
                    ...
                    if (item.getTransactions() != null) {
                        ...
                        String totalStr;
                        if (item.getSum() >= 0) {
                            totalStr = "Balance: $" + String.valueOf(new DecimalFormat("0.##").format(item.getSum()));
                            total.setTextColor(Color.rgb(0, 102, 32));
                        } else {
                            totalStr = "Balance: -$" + String.valueOf(new DecimalFormat("0.##").format(item.getSum()).replace("-", ""));
